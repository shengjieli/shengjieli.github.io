# 文章列表

![我自己的公众号-小黑同学](https://shengjieli.github.io/img/qrcode.jpg)

这是我的公众号，目前保持每周更新一篇的频率

---

### 公众号文章

[Django HTTPS部署指南](https://shengjieli.github.io/weixin/31)

[使用Django REST framework快速实现对数据库的增删查改](https://shengjieli.github.io/weixin/30)

[使用Gunicorn部署WSGI应用](https://shengjieli.github.io/weixin/29)

[使用uWSGI+Nginx部署WSGI应用](https://shengjieli.github.io/weixin/28)

[什么是WSGI、uWSGI和uwsgi](https://shengjieli.github.io/weixin/27)

[都9012年了，听说你还在用Postman发送HTTP请求？](https://shengjieli.github.io/weixin/26)

[不会前端，用Python也能生成好看的图表！](https://shengjieli.github.io/weixin/25)

[A厂招聘数据分析(图表版)](https://shengjieli.github.io/weixin/24)

[A厂招聘数据分析](https://shengjieli.github.io/weixin/23)

[用asycnio和aiohttp实现一个并发爬虫](https://shengjieli.github.io/weixin/22)

[Python的asyncio并发编程](https://shengjieli.github.io/weixin/21)

[分布式任务队列Celery](https://shengjieli.github.io/weixin/20)

[Redis 基础数据类型及操作](https://shengjieli.github.io/weixin/19)

[如何用Python是实现一个守护进程](https://shengjieli.github.io/weixin/18)

[subprocess子进程管理模块](https://shengjieli.github.io/weixin/17)

[并发编程终极武器concurrent.futures](https://shengjieli.github.io/weixin/16)

[Python多进程编程和进程间通信](https://shengjieli.github.io/weixin/15)

[多线程编程常用工具](https://shengjieli.github.io/weixin/14)

[Python多线程总结（Python2.7）](https://shengjieli.github.io/weixin/13)

[functools --- Higher-order functions and operations on callable objects](https://shengjieli.github.io/weixin/12)

[itertools — 为高效循环而创建迭代器的函数](https://shengjieli.github.io/weixin/11)

[Requests-HTML: HTML Parsing for Humans](https://shengjieli.github.io/weixin/10)

[不做“调库侠”，网页微信联系人信息获取和收发消息的实现](https://shengjieli.github.io/weixin/09)

[不做“调库侠”，网页微信登录过程的抓包分析及登录代码的简单实现](https://shengjieli.github.io/weixin/08)

[用Python和微信朋友聊天](https://shengjieli.github.io/weixin/07)

[Python抓取春节上映的8部影片的热门影评并生成词云](https://shengjieli.github.io/weixin/06)

[collections模块中相关类的使用（下）](https://shengjieli.github.io/weixin/05)

[Python的collections模块的使用（上）](https://shengjieli.github.io/weixin/04)

[神器Pipenv的简介与基本使用](https://shengjieli.github.io/weixin/03)

[url编码原理](https://shengjieli.github.io/weixin/02)

[Python老司机竟然又在编码问题上翻车了](https://shengjieli.github.io/weixin/01)

---

### 其他文章

[scrapy爬虫一定会用到的小技巧](https://shengjieli.github.io/2016/12/scrapy)

[go语言学习总结](https://shengjieli.github.io/2018-02-07-go)

[简单的小爬虫(go和python)](https://shengjieli.github.io/2018/02/spider)

